//! # md2htmlx-open
//!
//! GUI-subsystem launcher for [`md2htmlx`] intended for use as the Windows
//! Explorer "Open with" / right-click handler for `.md` files.
//!
//! `md2htmlx.exe` is a normal *console* subsystem binary. When Explorer
...
//! `md2htmlx.exe` itself as a windows-subsystem app would suppress the flash
//! but break terminal usage (no stdout to the parent shell, output races the
//! returning prompt under cmd/PowerShell).
//!
//! Instead, `md2htmlx-open.exe` is a tiny windows-subsystem wrapper that:
//!   1. Locates `md2htmlx[.exe]` next to itself.
...
//!
//! Net result: registering `md2htmlx-open.exe "%1"` in the Explorer file
...
//! `md2htmlx` binary; nothing about the subsystem flag applies there.
